    ...
    <div>
        <div class="x-breadcrumbs">
            <mu-breadcrumbs>
                ...
                <mu-breadcrumbs-item :disabled="true">会员权限列表</mu-breadcrumbs-item>
            </mu-breadcrumbs>
            <mu-button class="x-dwa-rt x-mt-10 x-mr-10" style="z-index:10;" color="primary" small @click="xzyn.go('/admin/authrulem/create')">新增</mu-button>
        </div>
        ...
        <mu-popover cover :open.sync="open" :trigger="trigger" class="x-p-15 x-yy-0" style="background: rgba(0,0,0,0);">
            <mu-button fab color="primary" small @click="xzyn.go('/admin/authrulem/edit?id='+this_data.id)">
...
export default {
    ...
    methods: {
        ...
        ajax_data: function(){
            ...
            _this.xzyn.ajax('/admin/authrulem/index','get',add_data,{},function(data,res){
        ...
        del_ajax: function(){
            ...
            this.$confirm('你确定要删除吗？', '提示').then((res)=>{
                if( res.result ){
                    this.xzyn.ajax('/admin/authrulem/delete','post',{id:this.this_data.id},{},function(data,res){
                        var new_data = [];
