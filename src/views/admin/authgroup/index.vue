    ...
    <div>
        <div class="x-breadcrumbs">
            <mu-breadcrumbs>
                ...
                <mu-breadcrumbs-item :disabled="true">角色管理</mu-breadcrumbs-item>
            </mu-breadcrumbs>
            <mu-button class="x-dwa-rt x-mt-10 x-mr-10" color="primary" small @click="xzyn.go('/admin/authgroup/create')">新增</mu-button>
        </div>
                ...
                <mu-paper :z-depth="4">
                    <mu-data-table stripe :columns="columns" :data="data_list" @row-click="row_click" :min-col-width="128">
                        <template slot-scope="scope">
                            ...
                            <td class="is-center">{{scope.row.module}}</td>
                            <td class="is-center">{{scope.row.level}}</td>
                            <td>{{scope.row.notation}}</td>
                            <td class="is-center">
        ...
        <mu-popover cover :open.sync="open" :trigger="trigger" class="x-p-15 x-yy-0" style="background: rgba(0,0,0,0);">
            <mu-button fab color="primary" small @click="xzyn.go('/admin/authgroup/edit?id='+this_data.id)">
                <mu-icon value=":fa fa-pencil-square-o" ></mu-icon>
            </mu-button>
            <mu-button fab color="success" class="x-mlr-5" small @click="xzyn.go('/admin/authgroup/authuser?id='+this_data.id)">
...
export default {
    ...
    data() {
        ...
        return {
            ...
            columns: [
                ...
                { title: '角色名称', name: 'title', align: 'center', width: 90},
                { title: '所属模块', name: 'module', align: 'center',width:100},
                { title: '角色等级', name: 'level', align: 'center',width:60 },
                { title: '角色描述', name: 'notation'},
    ...
    methods: {
        ...
        ajax_data: function(){
            ...
            _this.xzyn.ajax('/admin/authgroup/index','get',add_data,{},function(data,res){
        ...
        del_ajax: function(){
            ...
            this.$confirm('你确定要删除吗？', '提示').then((res)=>{
                if( res.result ){
                    this.xzyn.ajax('/admin/authgroup/delete','post',{id:this.this_data.id},{},function(data,res){
                        var new_data = [];
