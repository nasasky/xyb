    ...
    <div>
        <div class="x-breadcrumbs">
            <mu-breadcrumbs>
                ...
                <mu-breadcrumbs-item :disabled="true">编辑导航</mu-breadcrumbs-item>
            </mu-breadcrumbs>
        </div>
                            ...
                            <mu-col span="12" md="6">
                                <mu-form-item prop="type_id" label="导航分类" :rules="rules.type_id">
                                    <mu-select v-model="add_data.type_id" full-width @change="on_type_change" placeholder="请选择">
                                        <mu-option v-for="(item,index) in type_list" :key="index" :label="item+'（'+index+'）'" :value="index"></mu-option>
                                    </mu-select>
                                </mu-form-item>
                            </mu-col>
                            <mu-col span="12" md="6">
...
export default {
    ...
    methods: {
        ...
        add_ajax: function() {
            ...
            this.$refs.form.validate().then((result) => {
                if( result ){
                    ...
                    _this.xzyn.ajax('/admin/navigation/edit','post',_this.add_data,{},function(data,res){
                        _this.xzyn.go();
                    })
                }
            });
        ...
        ajax_data: function(){
            ...
            _this.xzyn.ajax('/admin/navigation/edit','get',add_data,{},function(data,res){
                _this.is_refreshing = false;
                if( typeof _this.type_list === 'string' ){
                    _this.type_list = data.typeList;
                }
                if( typeof _this.add_data.name !== 'string' ){
                    _this.add_data = data.navData;
                }
