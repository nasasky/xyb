    ...
    <div>
        <div class="x-breadcrumbs">
            <mu-breadcrumbs>
                ...
                <mu-breadcrumbs-item :disabled="false" to="/admin/apitype/index">API分类列表</mu-breadcrumbs-item>
                <mu-breadcrumbs-item :disabled="true">编辑API分类</mu-breadcrumbs-item>
            </mu-breadcrumbs>
        </div>
                ...
                <mu-row>
                    <mu-form ref="form" :model="add_data" label-position="top">
                        <mu-form-item class="x-mb-0 x-pb-0">
                            <mu-col span="12" md="6">
                                <mu-form-item prop="name" label="分类名称" :rules="rules.name">
                                    <mu-text-field v-model="add_data.name"></mu-text-field>
                                </mu-form-item>
                            </mu-col>
                            <mu-col span="12" md="6">
                                <mu-form-item prop="sorts" label="排序" :rules="rules.sorts">
...
export default {
    ...
    data() {
        ...
        return {
            ...
            rules: {
                name: [
                    { validate: (val) => !!val, message: '必须填写分类名称'}
                ],
                sorts: [
                    { validate: (val) => !!val, message: '必须填写排序'},
                ]
            }
    ...
    methods: {
        ...
        add_ajax: function() {
            ...
            this.$refs.form.validate().then((result) => {
                if( result ){
                    _this.xzyn.ajax('/admin/apitype/edit','post',_this.add_data,{},function(data,res){
                        _this.xzyn.go();
                    })
                }
            });
        ...
        ajax_data: function(){
            var _this = this;
            _this.xzyn.ajax('/admin/apitype/edit','get',{id: _this.$route.query.id},{},function(data,res){
                _this.add_data = data;
            },false)
        }
